...
|---|---|---|
| **Interface abstraction** | `ICache[K, V]` | Callers depend on `get`/`put`/`remove`, not on the hash-map + doubly-linked-list internals, so an LFU or TTL cache can drop in behind the same contract. |
| **Decorator** | `ConcurrentLruCache` wraps `LruCache` | Adds thread-safety (one lock) around every call without duplicating eviction logic — wrapper and wrapped share `ICache`, so the lock is a transparent layer. |
| **Sentinel node** | dummy head/tail in the DLL | Fixed boundary nodes eliminate null checks on insert/evict, making the O(1) move-to-front and evict-tail operations branch-free at the edges. |
