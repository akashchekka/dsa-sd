...
|---|---|---|
| **Strategy** | `ILogFormatter`, `ILogAppender` | Formatting ("how to render") and sinks ("where to write") vary independently; behind interfaces you add JSON/console/file support without editing the logger. |
| **Decorator** | `AsyncAppender(inner)` | Wraps any appender and adds off-thread writing while preserving the `ILogAppender` contract, so it composes transparently with any sink. |
| **Factory + cache (Flyweight)** | `LoggerFactory.get_logger(category)` | Centralizes construction and returns one shared `Logger` per category, so config lives in one place and duplicate loggers are avoided. |
| **Fluent Builder** | `add_appender(...).add_appender(...)` | Returning `self` lets callers assemble a configured factory in one readable chain. |
| **Poison pill (sentinel)** | `_SENTINEL` on the async queue | A worker blocked on `queue.get()` can't be interrupted by a flag; an in-band sentinel guarantees it wakes, drains, and exits cleanly. |
