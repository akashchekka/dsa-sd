...
|---|---|---|
| **Observer / Publish–Subscribe** | topic → list of `Subscription` | Fan-out to an arbitrary, changing subscriber set without the publisher knowing recipients. |
| **Strategy** | `INotificationChannel`, `IRetryPolicy` | "How to send" (email/SMS/push) and "when to retry" vary orthogonally; each is a swappable policy chosen at wiring time. |
| **Decorator** | retry policy wraps a channel send | Retry/backoff is layered around any channel, so transport ("how") and resilience ("when") evolve independently. |
