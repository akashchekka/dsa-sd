...
|---|---|---|
| **Producer–Consumer** | `submit()` producers, worker threads consumers, `queue.Queue` buffer | Decouples task submission from execution and caps concurrency at N workers; the blocking queue parks idle workers without busy-waiting. |
| **Future / Promise** | `Future` returned by `submit()` | Hands the caller a handle to a not-yet-ready result and lets them choose when to block, with task exceptions propagated naturally. |
| **Poison pill (sentinel)** | one `_SHUTDOWN` sentinel per worker | Because the queue is FIFO, all queued tasks run before the sentinels are reached, so workers drain then exit gracefully. |
