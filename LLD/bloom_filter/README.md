...
|---|---|---|
| **Strategy** | `IHashStrategy` → `DoubleHashingStrategy` | Index generation is the single axis that varies (double hashing, murmur, per-function salts). Isolating it behind an interface lets you swap hashing without touching the sizing math or bit storage. |
| **Dependency Injection** | `BloomFilter(hash_strategy=...)` | The filter receives its hasher instead of building one, so tests can inject a deterministic stub and production can pick any strategy at wiring time. |
| **Interface Segregation** | `IBloomFilter`, `IHashStrategy` | Two tiny contracts (membership vs. hashing) keep callers depending only on what they use, and open the door to Counting/Scalable variants behind the same `IBloomFilter`. |
